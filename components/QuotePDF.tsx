      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.row}>
          <View style={styles.column}>
            ...
            {quote.business.address && <Text style={styles.text}>{quote.business.address}</Text>}
            {quote.business.phone && <Text style={styles.text}>{quote.business.phone}</Text>}
            {quote.business.email && <Text style={styles.text}>{quote.business.email}</Text>}
            {quote.business.siret && (
              <Text style={styles.smallText}>SIRET: {quote.business.siret}</Text>
            )}
          </View>
        ...
        <View style={styles.table}>
          <View style={styles.tableHeader}>
            ...
            <Text style={[styles.tableHeaderCell, styles.col2]}>Prix unitaire</Text>
          ...
          {quote.items.map((item, index) => (
            <View key={index} style={styles.tableRow}>
              ...
              <Text style={[styles.tableCell, styles.col2]}>{item.price.toFixed(2)} €</Text>
              <Text style={[styles.tableCell, styles.col3]}>{item.quantity}</Text>
              <Text style={[styles.tableCellBold, styles.col4]}>{item.total.toFixed(2)} €</Text>
            </View>
          ))}
        </View>
        ...
        <View style={styles.footer}>
          ...
          <Text style={styles.footerText}>
            Ce devis est valable {quote.validUntil ? `jusqu'au ${new Date(quote.validUntil).toLocaleDateString('fr-FR')}` : 'pendant 30 jours'}
          </Text>
        </View>
      </Page>
    </Document>
