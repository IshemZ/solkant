    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-foreground/10 bg-white p-8 shadow-sm">
        ...
        <div className="mb-8 grid gap-8 sm:grid-cols-2">
          <div>
            <h2 className="mb-2 text-sm font-semibold uppercase text-gray-500">De</h2>
            <div className="text-gray-900">
              <p className="font-bold">{quote.business.name}</p>
              {quote.business.address && <p className="text-sm">{quote.business.address}</p>}
              {quote.business.phone && <p className="text-sm">{quote.business.phone}</p>}
              {quote.business.email && <p className="text-sm">{quote.business.email}</p>}
              {quote.business.siret && (
                <p className="mt-2 text-xs text-gray-500">SIRET: {quote.business.siret}</p>
              )}
            </div>
          </div>

          <div>
            <h2 className="mb-2 text-sm font-semibold uppercase text-gray-500">Pour</h2>
            <div className="text-gray-900">
              <p className="font-bold">
                {quote.client.firstName} {quote.client.lastName}
              </p>
              {quote.client.address && <p className="text-sm">{quote.client.address}</p>}
        ...
        <div className="mb-8">
          <table className="w-full">
            <thead className="border-b-2 border-gray-300">
              <tr>
                <th className="pb-2 text-left text-sm font-semibold text-gray-700">Description</th>
                <th className="pb-2 text-right text-sm font-semibold text-gray-700">Prix unitaire</th>
                <th className="pb-2 text-right text-sm font-semibold text-gray-700">Quantité</th>
                <th className="pb-2 text-right text-sm font-semibold text-gray-700">Total</th>
              </tr>
            </thead>
            <tbody>
        ...
        <div className="flex justify-end">
          <div className="w-64 space-y-2">
            ...
            <div className="flex justify-between border-t-2 border-gray-300 pt-2">
              ...
              <span className="text-xl font-bold text-gray-900">{quote.total.toFixed(2)} €</span>
            </div>
          </div>
        </div>
