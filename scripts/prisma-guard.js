...
const DANGEROUS_COMMANDS = {
  "migrate reset": {
    severity: "CRITIQUE",
    description:
      "Supprime toutes les données et réinitialise la base de données",
  ...
  "migrate resolve --rolled-back": {
    severity: "ÉLEVÉE",
    description:
      "Marque une migration comme annulée (peut causer des incohérences)",
    blockedIn: ["production"],
  },
};

// Allowed Prisma subcommands to prevent command injection
...
function checkDangerousCommand(command, env) {
  for (const [dangerousCmd, config] of Object.entries(DANGEROUS_COMMANDS)) {
    if (command.includes(dangerousCmd)) {
      const isBlocked =
        config.blockedIn.includes(env.name) ||
      ...
      if (isBlocked) {
        ...
        console.error(
          "  - Contactez l'équipe DevOps si vous devez vraiment exécuter cette commande\n"
        );
        process.exit(1);
      }
    }
  ...
  for (const [cmd, config] of Object.entries(REQUIRES_CONFIRMATION)) {
    if (command.includes(cmd)) {
      const needsConfirmation =
        config.requiresConfirmation.includes(env.name) ||
        (config.requiresConfirmation.includes("production") &&
          env.isProduction) ||
      ...
      if (needsConfirmation) {
        ...
        if (!skipConfirm) {
          console.error(
            "❌ Commande bloquée. Utilisez --force pour passer outre (non recommandé).\n"
          );
          process.exit(1);
        }

        console.warn(
          "⚠️  Confirmation ignorée avec --force. Procédez avec prudence!\n"
        );
      }
    }
  }
...
function main() {
  ...
  // Validate the first argument is an allowed Prisma command
  const primaryCommand = args[0];
  if (!ALLOWED_PRISMA_COMMANDS.includes(primaryCommand)) {
    console.error(
      `\n❌ Commande Prisma non autorisée: ${primaryCommand}\n`
    );
    console.error(
      `Commandes autorisées: ${ALLOWED_PRISMA_COMMANDS.join(", ")}\n`
    );
    process.exit(1);
  }

  // Validate all arguments contain only safe characters (alphanumeric, dashes, underscores, dots, equals)
  const safeArgPattern = /^[a-zA-Z0-9_\-=./:]+$/;
  for (const arg of args) {
    if (!safeArgPattern.test(arg)) {
      console.error(
        `\n❌ Argument non sécurisé détecté: ${arg}\n`
      );
      console.error(
        "Les arguments ne peuvent contenir que des caractères alphanumériques, tirets, underscores, points et signes égal.\n"
      );
      process.exit(1);
    }
  }
  ...
  // Si tout est OK, exécuter la commande avec spawnSync pour éviter l'injection de commandes
  try {
